...
> Pipeline live : un avatar VRM piloté par LLM via tool_calls,
> parle en streaming TTS MiniMax WS, le tout dans un design
...
- **`identity.py`** — `VisitorIdentity` (ip_hash) vs `OperatorIdentity`
  (username, jti, session_id). Barrière de type : les brains opérateur
  rejettent tout appel sans `OperatorIdentity`.
...
- **`body_router.py`** — Dispatcher des tool_calls LLM directeur :
...
- **`brain_director_*.py`** — Brains directeur LLM (Anthropic, MiniMax, Ollama,
  OpenAI). Boucle tool-use, parse `tool_calls[]`. Remplace les anciens
  HermesAgentBrain/HermesEmbodiedBrain (retirés en PR #91).
...
- **`desktop/desktopState.tsx`** — Context + reducer. Actions `nextZ` +
...
- **`desktop/DesktopWindow.tsx`** — Une fenêtre. Animation char-par-char 45
  chars/sec avec cursor pulse. Commandé uniquement par le LLM directeur.
...
- **`OperatorPanel.tsx`** — Top-right collapse. Toggle debug captions.
...
- `viewer.count`
- `error.moderation` / `queue.rejected`
