...
export default function AdminUsersPage() {
  ...
  return (
    <>
      <Meta title="Utilisateurs — Shugu Admin" />
      <AdminShell
        active="users"
        title="Utilisateurs"
                  ...
                  sub={
                    <span className="block text-[12px] opacity-65">
                      ...
                      {u.vip_until && u.vip_active && (
                        <span className="ml-2 opacity-80">· VIP jusqu'au {formatDate(u.vip_until)}</span>
                      )}
                      <span className="ml-2 opacity-40">· créé {formatDate(u.created_at)}</span>
                    </span>
                  }
                  trailing={
                    <div className="flex items-center gap-2">
